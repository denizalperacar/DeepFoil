...
class AirfoilPredictor(torch.nn.Module):
    def __init__(self):
        ...
        self.l0  = torch.nn.Linear(800, 800, bias=True)
        ...
        self.l6  = torch.nn.Linear(800, 800, bias=True)
        self.l7  = torch.nn.Linear(800, 800, bias=True)
        self.l8  = torch.nn.Linear(800, 800, bias=True)
        self.l9  = torch.nn.Linear(800, 800, bias=True)
        self.l10 = torch.nn.Linear(800, 800, bias=True)
        self.l11 = torch.nn.Linear(800, 800, bias=True)
        self.l12 = torch.nn.Linear(800, 800, bias=True)
        self.l13 = torch.nn.Linear(800, 800, bias=True)
        self.l14 = torch.nn.Linear(800, 800, bias=True)
        self.l15 = torch.nn.Linear(800, 800, bias=True)
        self.l16 = torch.nn.Linear(800, 800, bias=True)
        self.l17 = torch.nn.Linear(800, 800, bias=True)
        self.l18 = torch.nn.Linear(800, 800, bias=True)
        self.l19 = torch.nn.Linear(800, 800, bias=True)
        self.l20 = torch.nn.Linear(800, 800, bias=True)
        self.l21 = torch.nn.Linear(800, 800, bias=True)
        self.l22 = torch.nn.Linear(800, 800, bias=True)
        self.l23 = torch.nn.Linear(800, 1, bias=True)
    ...
    def forward(self, x):
        ...
        x   = self.a(self.l2(x1))
        x   = self.a(self.l3(x))
        x2  = self.a(self.l4(x)) + x1#+ self.s(self.r1(x1))
        x   = self.a(self.l5(x2))
        x   = self.a(self.l6(x))
        x3  = self.a(self.l7(x)) + x2#+ self.s(self.r2(x2))
        x   = self.a(self.l8(x3))
        x   = self.a(self.l9(x))
        x4  = self.a(self.l10(x)) + x3#+ self.s(self.r3(x3))
        x   = self.a(self.l11(x4))
        x   = self.a(self.l12(x))
        x5  = self.a(self.l13(x)) +x4#+ self.s(self.r4(x4))
        ...
        x   = self.a(self.l21(x))
        x8  = self.a(self.l22(x)) + x7
        x   = self.a(self.l23(x8))
        return x
...
# load_mode;
model.load_state_dict(torch.load('net_38'))
...
def train_dev_test_sets(df_loc='tr_raw_af_data', s_train=0.99,
                        alpha_range=[-5, 15], Re_range=[3000000, 10000000],
